...
function normalizeStyle(value) {
  return ['classic', 'solid', 'status', 'tech'].includes(value) ? value : 'classic';
}
...
export function renderBadgeSvg(input = {}) {
  ...
  if (options.style === 'solid') {
    const text = escapeXml(`${options.label} ${options.message}`);
    const width = textLength(`${options.label} ${options.message}`) + 22;
    return `<!-- This is build by svg tool, see more here: https://github.com/HammCn/svg-badge-tool -->
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="22" role="img" aria-label="${key}: ${value}" data-style="solid" style="user-select:text;-webkit-user-select:text">
  <title>${key}: ${value}</title>
  <rect width="${width}" height="22" rx="3" fill="${options.color}"/>
  <text x="${Math.round(width / 2)}" y="15" fill="#ffffff" text-anchor="middle" font-family="Verdana,Geneva,DejaVu Sans,sans-serif" font-size="11">${text}</text>
</svg>`;
  }
  if (options.style === 'status') {
    const width = lenKey + lenValue + 34;
    return `<!-- This is build by svg tool, see more here: https://github.com/HammCn/svg-badge-tool -->
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="22" role="img" aria-label="${key}: ${value}" data-style="status" style="user-select:text;-webkit-user-select:text">
  <title>${key}: ${value}</title>
  <rect width="${width}" height="22" rx="11" fill="#ffffff" stroke="#d1d5db"/>
  ...
  <text x="${lenKey + 28}" y="15" fill="${options.color}" font-family="Verdana,Geneva,DejaVu Sans,sans-serif" font-size="11" font-weight="700">${value}</text>
</svg>`;
  }
  if (options.style === 'tech') {
    const leftWidth = lenKey + 18;
    const rightWidth = lenValue + 22;
    const width = leftWidth + rightWidth;
    return `<!-- This is build by svg tool, see more here: https://github.com/HammCn/svg-badge-tool -->
<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="24" role="img" aria-label="${key}: ${value}" data-style="tech" style="user-select:text;-webkit-user-select:text">
  <title>${key}: ${value}</title>
  <rect x="0.5" y="0.5" width="${width - 1}" height="23" rx="12" fill="#ffffff" stroke="${options.color}"/>
  <rect x="${leftWidth}" y="3" width="${rightWidth - 4}" height="18" rx="9" fill="${options.color}"/>
  <text x="10" y="16" fill="#1f2937" font-family="Verdana,Geneva,DejaVu Sans,sans-serif" font-size="11">${key}</text>
  <text x="${leftWidth + Math.round((rightWidth - 4) / 2)}" y="16" fill="#ffffff" text-anchor="middle" font-family="Verdana,Geneva,DejaVu Sans,sans-serif" font-size="11" font-weight="700">${value}</text>
</svg>`;
  }
