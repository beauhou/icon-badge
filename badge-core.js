...
function normalizeStyle(value) {
  return ['classic', 'flat', 'pill', 'outline'].includes(value) ? value : 'classic';
}
...
export function renderBadgeSvg(input = {}) {
  ...
  const border = isOutline
    ? `<rect x="0.5" y="0.5" width="${width - 1}" height="19" rx="${styleRadius}" fill="none" stroke="${options.color}"/>`
    : '';
  const divider = isOutline
    ? `<line x1="${leftWidth}" y1="4" x2="${leftWidth}" y2="16" stroke="#d1d5db"/>`
    : '';
